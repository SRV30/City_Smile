    ...
    <section id="testimonials" className="container scroll-mt-28 py-10 lg:py-14">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-3">
        {testimonials.slice(0, 3).map((item) => (
          <article key={item.patientName} className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
            <div className="text-4xl font-black leading-none text-blue-600">“</div>
            <p className="mt-2 min-h-24 text-sm leading-7 text-slate-700">{item.quote}</p>
            <div className="mt-5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <img src={item.avatar} alt={item.patientName} className="h-11 w-11 rounded-full object-cover" />
                <div>
                  <p className="text-sm font-extrabold text-blue-600">{item.patientName}</p>
                  {item.treatment && <p className="text-xs text-slate-500">{item.treatment}</p>}
                </div>
              </div>
