...
const TestimonialsSection = ({
  ...
  // Memoize grouped testimonials based on window size
  // For mobile: 1 card, tablet: 2 cards, desktop: 3 cards
    ...
    return (
      <section id="testimonials" className="container scroll-mt-28 py-10 lg:py-14">
        <div className="rounded-3xl bg-red-50 p-10 text-center border border-red-100">
          ...
          <h3 className="mt-4 text-xl font-bold text-slate-900">Oops! Failed to load testimonials</h3>
          <p className="mt-2 text-slate-600">We encountered an error while fetching patient stories. Please try again.</p>
          <button
            onClick={onRetry}
    ...
    return (
      <section id="testimonials" className="container scroll-mt-28 py-10 lg:py-14">
        <div className="rounded-3xl bg-blue-50/50 p-10 text-center border border-blue-100/50">
          ...
          <h3 className="mt-4 text-xl font-bold text-slate-900">No testimonials available yet</h3>
          <p className="mt-2 text-slate-600">Be the first patient to share your experience with us!</p>
          <button
      ...
      >
        ...
        >
          {Array.from({ length: totalPages }).map((_, pageIndex) => (
            <div key={pageIndex} className="grid w-full shrink-0 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {testimonials
                .slice(pageIndex * itemsPerPage, (pageIndex + 1) * itemsPerPage)
                .map((item, index) => (
                  <article key={item._id || index} className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100 flex flex-col h-full">
                    <div className="text-4xl font-black leading-none text-blue-600">“</div>
                    <p className="mt-2 flex-grow text-sm leading-7 text-slate-700">{item.review}</p>
